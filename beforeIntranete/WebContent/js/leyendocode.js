...
(function(ap, t) {
	...
	ap.fn.fullCalendar = function(aI) {
		...
		aI = ap.extend(true, {}, J, (aI.isRTL || aI.isRTL === t && J.isRTL) ? P
				: {}, aI);
	...
	function q(aY, a5, a8) {
		...
		function a6() {
			...
			aU = ap("<div class='fc-content' style='position:relative'/>")
					.prependTo(aY);
			aL = new Q(bn, a5);
			bf = aL.render();
			if (bf) {
		...
		function aP(bz) {
			if (!aT || bz != aT.name) {
				...
				if (aT) {
					...
					aT = bv[bz] = new ab[bz](bx = aQ = ap(
							"<div class='fc-view fc-view-" + bz
		...
		function bc(bx, by) {
			if (a5[bx]) {
				return a5[bx].apply(by || bl, Array.prototype.slice.call(
						arguments, 2))
			}
		}
		if (a5.droppable) {
	...
	function Q(aJ, aS) {
		...
		function aH() {
			...
			var aT = aS.header;
			if (aT) {
				aK = ap("<table class='fc-header' style='width:100%'/>")
						.append(
								ap("<tr/>").append(aQ("left")).append(
										aQ("center")).append(aQ("right")));
				return aK
			}
		}
		function aN() {
															...
															if (aY) {
																...
																var a1 = ap(
																		"<span class='fc-button fc-button-"
																				...
																				+ aO
																				+ "-state-default'>"
																				+ (a2 ? "<span class='fc-icon-wrap'><span class='ui-icon ui-icon-"
																						+ a2
																						+ "'/></span>"
																						: a3)
																				+ "</span>")
	...
	function p(aM, aU) {
		...
		function aZ(bh) {
			var bf, bd = aX.length, bi, bc = aG().defaultEventEnd, bg = bh.start
					- bh._start, be = bh.end ? (bh.end - (bh._end || bc(bh)))
					: 0;
			for (bf = 0; bf < bd; bf++) {
				bi = aX[bf];
	...
	aw.formatDate = y;
	aw.formatDates = h;
	var N = [ "sun", "mon", "tue", "wed", "thu", "fri", "sat" ], ar = 86400000, ah = 3600000, S = 60000;
	...
	function l(aK, aH) {
		...
		if (aH || !aG[13]) {
			...
			aJ.setUTCHours(aG[7] || 0, aG[8] || 0, aG[10] || 0,
					aG[12] ? Number("0." + aG[12]) * 1000 : 0);
			if (aG[14]) {
	...
	function h(aS, aR, aQ, aT) {
		aT = aT || J;
		var aH = aS, aJ = aR, aK, aL = aQ.length, aN, aI, aP, aM = "";
		for (aK = 0; aK < aL; aK++) {
			aN = aQ.charAt(aK);
			if (aN == "'") {
	...
	function r(aH, aG) {
		return (aG.msLength - aH.msLength) * 100
				+ (aH.event.start - aG.event.start)
	}
	function L(aH, aG) {
		return aH.end > aG.start && aH.start < aG.end
	}
	function Z(aS, aM, aI, aL) {
		var aJ = [], aN, aP = aS.length, aH, aQ, aO, aR, aT, aG, aK;
		for (aN = 0; aN < aP; aN++) {
			aH = aS[aN];
			aQ = aH.start;
			aO = aM[aN];
	...
	function ae(aI) {
		var aM = [], aL, aG = aI.length, aH, aK, aN, aJ;
		for (aL = 0; aL < aG; aL++) {
			aH = aI[aL];
			aK = 0;
			while (true) {
	...
	function ad(aG) {
		return (parseFloat(ap.css(aG[0], "paddingLeft", true)) || 0)
				+ (parseFloat(ap.css(aG[0], "paddingRight", true)) || 0)
	}
	function ac(aG) {
		return (parseFloat(ap.css(aG[0], "marginLeft", true)) || 0)
				+ (parseFloat(ap.css(aG[0], "marginRight", true)) || 0)
	}
	function n(aG) {
		return (parseFloat(ap.css(aG[0], "borderLeftWidth", true)) || 0)
				+ (parseFloat(ap.css(aG[0], "borderRightWidth", true)) || 0)
	}
	function C(aH, aG) {
		return v(aH) + am(aH) + (aG ? i(aH) : 0)
	}
	function v(aG) {
		return (parseFloat(ap.css(aG[0], "paddingTop", true)) || 0)
				+ (parseFloat(ap.css(aG[0], "paddingBottom", true)) || 0)
	}
	function i(aG) {
		return (parseFloat(ap.css(aG[0], "marginTop", true)) || 0)
				+ (parseFloat(ap.css(aG[0], "marginBottom", true)) || 0)
	}
	function am(aG) {
		return (parseFloat(ap.css(aG[0], "borderTopWidth", true)) || 0)
				+ (parseFloat(ap.css(aG[0], "borderBottomWidth", true)) || 0)
	}
	function G(aH, aG) {
	...
	function B(aK, aG) {
		...
		var aJ = aG.split(/(?=[A-Z])/), aI = aJ.length - 1, aH;
		for (; aI >= 0; aI--) {
	...
	function aB(aG) {
		return aG.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g,
				"&gt;").replace(/'/g, "&#039;").replace(/"/g, "&quot;")
				.replace(/\n/g, "<br />")
	}
	function E(aG) {
		return aG.id
				...
				+ aG.className
				+ "/"
				+ aG.style.cssText.replace(
						/(^|;)\s*(top|left|width|height)\s*:[^;]*/ig, "")
	}
	function aF(aG) {
		aG.attr("unselectable", "on").css("MozUserSelect", "none").bind(
				"selectstart.ui", function() {
					return false
				})
	}
	function c(aG) {
		aG.children().removeClass("fc-first fc-last").filter(":first-child")
				.addClass("fc-first").end().filter(":last-child").addClass(
						"fc-last")
	}
	function I(aG, aH) {
		aG.each(function(aI, aJ) {
			aJ.className = aJ.className.replace(/^fc-\w*/, "fc-"
					+ N[aH.getDay()])
		})
	}
	function O(aH, aI) {
		...
		var aO = aH.backgroundColor || aN || aG.backgroundColor || aL
				|| aI("eventBackgroundColor") || aK;
		var aJ = aH.borderColor || aN || aG.borderColor || aL
				|| aI("eventBorderColor") || aK;
	...
	function x(aY, bs, aP) {
		...
		function aH() {
			aT = ap("<div style='position:absolute;z-index:8;top:0;left:0'/>")
					.appendTo(aY)
		}
		function a8(bU) {
			var bQ = "";
			...
			for (bP = 0; bP < aL; bP++) {
				bN = a4(0, bP);
				bQ += "<th class='fc-day-header fc-" + N[bN.getDay()] + " "
						+ bL + "'/>"
			}
			bQ += "</tr></thead><tbody>";
			for (bP = 0; bP < a7; bP++) {
				...
				for (bO = 0; bO < aL; bO++) {
					...
					bQ += "<td class='" + bT.join(" ") + "' data-date='"
							+ bq(bN, "yyyy-MM-dd") + "'><div>";
					if (bU) {
						bQ += "<div class='fc-day-number'>" + bN.getDate()
								+ "</div>"
					}
	...
	function ai() {
		...
		function aQ(bf) {
			var bc = aK(), be = aW(), a6 = K(aR.visStart), a4 = ay(K(a6), be), bd = ap
					.map(bf, ak), bb, bg, ba, a5, a8, a9, a7 = [];
			for (bb = 0; bb < bc; bb++) {
				bg = ae(Z(bf, bd, a6, a4));
	...
	function D(b9, aY, bc) {
		...
		function ba() {
			...
			bE = ap(
					...
					.appendTo(bo);
			aN = ap("<div style='position:absolute;z-index:8;top:0;left:0'/>")
					.appendTo(bE);
		...
		function bP(ci) {
			if (!bC("selectable")) {
				var cg = Math.min(bH - 1, Math.floor((ci.pageX
						- bQ.offset().left - bk)
						/ aQ));
				var cf = bx(cg);
		...
		function cc(cf, cj) {
			...
			var ce = bC("slotMinutes"), ci = cj.getHours() * 60
					+ cj.getMinutes() - bf, ch = Math.floor(ci / ce), cg = bd[ch];
			if (cg === t) {
	...
	function R() {
		...
		function aG(bv) {
			var bx = ae(Z(bv, ap.map(bv, ak), bh.visStart, bh.visEnd)), bu, bw = bx.length, by, bt, bs, br = [];
			for (bu = 0; bu < bw; bu++) {
				by = bx[bu];
		...
		function aZ(bD) {
			var bC = bd(), bA = a9(), bu = aH(), bB = g(K(bh.visStart), bA), by = ap
					.map(bD, bo), bz, bt, bx, br, bv, bw, bs = [];
			for (bz = 0; bz < bC; bz++) {
		...
		function aQ(bH, bI) {
			var bN, bQ = bH.length, bP, bL, bR, bC, bz, by, bt, bA, bx, bK, br, bu, bB = "", bS, bO, bw, bs = {}, bV = {}, bU, bT, bF, bE, bM = bp(), bv, bG, bD, bJ = bd();
			if (bv = aV("isRTL")) {
				bG = -1;
				bD = bJ - 1
			} else {
			...
			for (bN = 0; bN < bQ; bN++) {
				bP = bH[bN];
				if (bO = bP.element) {
					bO[0].style.width = Math.max(0, bP.outerWidth - bP.hsides)
							+ "px";
					bE = Math.max(0, bP.outerHeight - bP.vsides);
					bO[0].style.height = bE + "px";
					bL = bP.event;
					if (bP.contentTop !== t && bE - bP.contentTop < 10) {
						bO.find("div.fc-event-time").text(
								bf(bL.start, aV("timeFormat")) + " - "
										+ bL.title);
		...
		function a3(bw, br) {
			...
			bv += " class='"
					...
					+ aB(aX(bw.start, bw.end, aV("timeFormat")))
					+ "</div><div class='fc-event-title'>" + aB(bw.title)
					...
					.draggable({
														...
														if (br) {
															if (bE) {
																...
																W(
																		...
																		bt
																				* Math
																						.round((bs.end ? ((bs.end - bs.start) / S)
																								: aV("defaultEventMinutes"))
																								/ bD));
																bB
																		.draggable(
																				"option",
						...
						stop : function(bG, bH) {
							...
							if (bA) {
								...
								var bF = 0;
								if (!bE) {
									bF = Math.round((bB.offset().top - a1()
											.offset().top)
											/ bt)
											* bD
											+ bz
											- (bs.start.getHours() * 60 + bs.start
													.getMinutes())
								}
			...
			bt
					.resizable({
						...
						resize : function(by, bz) {
							bu = Math
									.round((Math.max(bx, bt.height()) - bz.originalSize.height)
											/ bx);
							if (bu != br) {
								bw
										.text(aX(bv.start,
		...
		for (aI = aK.length - 1; aI > 0; aI--) {
			...
			for (aH = 0; aH < aM.length; aH++) {
				...
				for (aG = 0; aG < aK[aI - 1].length; aG++) {
					aJ = aK[aI - 1][aG];
					if (L(aL, aJ)) {
						aJ.forward = Math.max(aJ.forward || 0,
								(aL.forward || 0) + 1)
					}
				}
			}
	...
	function az(aK, aR, aW) {
		...
		function aS(a8, a9) {
			return aR.trigger.apply(aR, [ a8, a9 || aQ ].concat(
					Array.prototype.slice.call(arguments, 2), [ aQ ]))
		}
		function aV(a8) {
		...
		function aH(a9, a8) {
			a8.click(
					function(ba) {
						if (!a8.hasClass("ui-draggable-dragging")
								&& !a8.hasClass("ui-resizable-resizing")) {
		...
		function aT(bb, ba, bd) {
			var bc = aN[bb._id], a9, a8 = bc.length;
			for (a9 = 0; a9 < a8; a9++) {
				if (!ba || bc[a9][0] != ba[0]) {
					bc[a9][bd]()
				}
	...
	function V() {
		...
		function a0(bo) {
			...
			for (bt = 0; bt < bz; bt++) {
				...
				bu += " class='" + bq.join(" ")
						+ "' style='position:absolute;z-index:8;left:" + bp
						+ "px;" + bw + "'><div class='fc-event-inner'>";
				if (!bm.allDay && bs.isStart) {
					bu += "<span class='fc-event-time'>"
							+ aB(aT(bm.start, bm.end, aR("timeFormat")))
							+ "</span>"
				}
				bu += "<span class='fc-event-title'>" + aB(bm.title)
						+ "</span></div>";
				if (bs.isEnd && be(bm)) {
					bu += "<div class='ui-resizable-handle ui-resizable-"
							+ (bv ? "w" : "e") + "'>&nbsp;&nbsp;&nbsp;</div>"
				}
				bu += "</" + (bn ? "a" : "div") + ">";
				bs.left = bp;
		...
		function aK(bk) {
			...
			for (bm = 0; bm < bn; bm++) {
				...
				bl = bj.element;
				if (bl) {
					bl[0].style.width = Math.max(0, bj.outerWidth - bj.hsides)
							+ "px"
				}
			}
		}
	...
	function j(aH) {
		...
		aG.right = function(aM) {
			return aK[aM] = aK[aM] === t ? aG.left(aM) + aJ(aM).width()
					: aK[aM]
		};
		aG.clear = function() {
			aI = {};
...
PrimeFaces.widget.Schedule = PrimeFaces.widget.DeferredWidget
		.extend({
						...
						if (g) {
							var e = {
								params : [ {
									...
									value : b.getTime() - b.getTimezoneOffset()
											* 60000 - a.cfg.offset
								} ]
							};
							g.call(a, b, e)
			...
			setupEventSource : function() {
				var a = this, b = new Date().getTimezoneOffset() * 60000
						+ this.cfg.offset;
					...
					var d = {
						...
						onsuccess : function(m) {
							...
							for ( var h = 0; h < l.length; h++) {
								var o = l.eq(h), n = o.attr("id"), j = o.get(0).childNodes[0].nodeValue;
								if (n == a.id) {
									var g = $.parseJSON(j).events;
									e(g)
								} else {
									PrimeFaces.ajax.AjaxUtils.updateElement
											.call(this, n, j)
								}
							}
							PrimeFaces.ajax.AjaxUtils.handleResponse.call(this,
									k);
							return true
						}
					};
